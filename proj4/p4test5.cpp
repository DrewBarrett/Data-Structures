...
int main() {
   ...
   for (int i=0 ; i < numItems ; i+=7) {
      tempCity = cities[i] ; 
      PopH.deleteItem(tempCity,City::sameCity) ;
      ElevH.deleteItem(tempCity,City::sameCity) ;
   }
   ...
   PopH.dump();
   sanityCheck(PopH,City::smallerPopulation,City::biggerPopulation) ;
   
   cout << "\nChecking Elevation MedianHeap...\n" ;
   cout << "   min    = " << ElevH.getMin() << endl ;
   cout << "   median = " << ElevH.getMedian() << endl ;
   cout << "   max    = " << ElevH.getMax() << endl ;
   sanityCheck(ElevH,City::smallerElevation,City::biggerElevation) ;
